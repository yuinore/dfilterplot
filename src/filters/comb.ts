...
export class CombFilterDesign implements FilterDesignBase {
   ...
   * @returns 極・零点・ゲイン
   */
  private generateFeedforwardComb(delay: number, gain: number): FilterGenerationResult {
    // 零点: z^M = -α
   ...
   * @returns 極・零点・ゲイン
   */
  private generateFeedbackComb(delay: number, gain: number): FilterGenerationResult {
    // 極: z^M = α
